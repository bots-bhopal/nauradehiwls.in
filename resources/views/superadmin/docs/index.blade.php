                        ...
                        <div class="card-header d-flex">
                            <h3 class="card-title my-auto" style="font-size: 18px; font-weight: 600;">
                                ...
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip"
                                    data-placement="top" title="Total Number of Other Document's"
                                    style="font-size: 14px; font-weight: 500;">{{ $documents->count() }}</span>
                            </h3>
                            <a class="btn btn-sm btn-primary ml-auto" href="{{ route('superadmin.document.create') }}"><i
                                    class="nav-icon fas fa-plus-circle"
                                    style="margin-right: 5px;"></i>{{ __('panel.document-btn-add') }}</a>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="tblDocs" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">{{ __('panel.document-tbl-qr') }}</th>
                                ...
                                @foreach ($documents as $key => $document)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <img src="data:image/png;base64,{!! DNS2D::getBarcodePNG($document->url, 'QRCODE', 4, 4) !!}" alt="barcode" />
                                            {{-- {!! DNS2D::getBarcodeHTML($document->url, 'QRCODE') !!} --}}
                                            {{-- {!! QrCode::size(100)->eyeColor(0, 28, 141, 52, 0, 0, 0)->generate($document->url) !!} --}}
                                        </td>
                                        <td class="text-center">
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('superadmin.document.edit', $document->id) }}"
                                                class="btn btn-sm btn-primary mt-2"><i class="far fa-edit"
                                            ...
                                            <form id="delete-form-{{ $document->id }}"
                                                action="{{ route('superadmin.document.destroy', $document->id) }}"
                                                method="POST" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>

                                            <a href="{{ route('nwls.docs.download', [$document->original_filename, $document->status]) }}"
                                                class="btn btn-sm btn-success mt-2"><i class="fa fa-download"
