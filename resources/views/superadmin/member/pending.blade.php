...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('panel.dash-pending-member') }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{route('superadmin.dashboard')}}">{{ __('panel.home') }}</a></li>
                        <li class="breadcrumb-item active">{{ __('panel.dash-pending-member') }}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header d-flex">
                            <h3 class="card-title my-auto" style="font-size: 18px; font-weight: 600;">
                                {{ __('panel.dash-pending-member') }}
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip" data-placement="top" title="Total Number of Pending Members" style="font-size: 14px; font-weight: 500;">{{ $pendingMember->count() }}</span>
                            </h3>

                            {{-- <a class="btn btn-sm btn-primary ml-auto" href="{{route('superadmin.member.create')}}"><i class="nav-icon fas fa-plus-circle" style="margin-right: 5px;"></i>{{ __('panel.member-btn-add') }}</a> --}}
                        ...
                        <div class="card-body">
                            <table id="tblMember" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach ($pendingMember as $key=>$member)
                                        <tr>
                                            ...
                                            <td>
                                                <input data-id="{{$member->id}}" class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-size="small" data-toggle="toggle" data-style="slow" data-on="Showing" data-off="Hidden" {{ $member->status ? '' : 'checked' }}>
                                            </td>
                                            <td class="text-center w-25">
                                                <a href="{{ route('superadmin.member.edit', $member->id) }}" class="btn btn-sm btn-info mt-1"><i class="nav-icon fas fa-edit" style="margin-right: 5px;"></i>{{ __('panel.member-btn-edit') }}</a>
                                                ...
                                                <form id="delete-form-{{ $member->id }}" action="{{ route('superadmin.member.destroy', $member->id) }}" method="POST" style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger" id="delMember">{{ __('panel.member-modal-btn-yes') }}</button>
                </div>
            </div>
        </div>
    </div>
