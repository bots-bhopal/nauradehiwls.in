                        ...
                        <div class="card-header d-flex">
                            <h3 class="card-title my-auto" style="font-size: 18px; font-weight: 600;">
                                ...
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip"
                                    data-placement="top" title="Total Number of Tender's"
                                    style="font-size: 14px; font-weight: 500;">{{ $tenders->count() }}</span>
                            </h3>
                            <a class="btn btn-sm btn-primary ml-auto" href="{{ route('serveradmin.tender.create') }}"><i
                                    class="nav-icon fas fa-plus-circle"
                                    style="margin-right: 5px;"></i>{{ __('panel.tender-btn-add') }}</a>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                                ...
                                <tbody>
                                    @foreach ($tenders as $key => $tender)
                                        <tr>
                                            ...
                                            <td width="10%" class="text-center">
                                                @if ($tender->file_extension == 'doc' || $tender->file_extension == 'docx')
                                                    <span class="badge badge-primary"
                                                        style="font-size: 14px;">{{ HumanReadable::bytesToHuman($tender->file_size) }}</span>
                                                @endif
                                            ...
                                            <td>
                                                <input data-id="{{ $tender->id }}" class="toggle-class" type="checkbox"
                                                    data-onstyle="success" data-offstyle="danger" data-size="small"
                                                    data-toggle="toggle" data-style="slow" data-on="Approved"
                                                    data-off="Pending" {{ $tender->status ? '' : 'checked' }}>
                                            </td>
                                            <td class="text-center">
                                                <a href="{{ route('serveradmin.tender.edit', $tender->id) }}"
                                                ...
                                                <button type="button" class="btn btn-sm btn-danger mt-1"
                                                    onclick="deleteTender({{ $tender->id }})" data-toggle="tooltip"
                                                    data-placement="top" title="Delete"
                                                    style="font-size: 14px; font-weight: 500;"><i
                                                        class="fa fa-trash-alt"></i> Delete</button>
                                                <form id="delete-form-{{ $tender->id }}"
                                                    action="{{ route('serveradmin.tender.destroy', $tender->id) }}"
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                        data-dismiss="modal">{{ __('panel.tender-modal-btn-cancel') }}</button>
                    <button type="button" class="btn btn-danger"
                        id="delTender">{{ __('panel.tender-modal-btn-yes') }}</button>
                </div>
            </div>
        </div>
    </div>
