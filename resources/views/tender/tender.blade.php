                        ...
                        <div class="col-md-12 col-lg-4 mb-80">
                            <div class="about-block gray-bg padding-40 clearfix">
                                ...
                                <div class="text-box">
                                    ...
                                    <a href="{{ route('str.tender.download', $tender->original_filename) }}"
            ...
            @if ($tenders->lastPage() > 1)
                <ul>
                    @if ($tenders->onFirstPage())
                        <li><a class="{{ $tenders->currentPage() == 1 ? 'disabled' : '' }}"><i
                    ...
                    @for ($i = 1; $i <= $tenders->lastPage(); $i++)
                        <li>
                            <a class="{{ $tenders->currentPage() == $i ? ' active' : '' }}"
                                href="{{ $tenders->url($i) }}">{{ $i }}</a>
                        </li>
                    @endfor

                    @if ($tenders->hasMorePages())
                        <li><a class="{{ $tenders->currentPage() == $tenders->lastPage() ? ' disabled' : '' }}"
                                href="{{ $tenders->nextPageUrl() }}"><i class="fa fa-angle-right"></i></a></li>
                    @else
