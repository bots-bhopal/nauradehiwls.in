...
class DocumentRepository implements DocumentInterface
{
    ...
    public function storeDocument($data)
    {
        ...
        // Generate QR Code
        // $url = config('app.url') . '/docs-download/' . $file->getClientOriginalName();
        // $path = Storage::path('public/doc_qr_codes/');

        // $qr = new DNS2D();
        // $qr->setStorPath($path);
        // $qr->getBarcodeHTML($url, 'QRCODE');
    ...
    public function download($filename, $status)
    {
        $file_path = public_path('storage/document/docs/') . $filename;
        if (file_exists($file_path) && $status == '0') {
            // Send Download
