...
class NewsController extends Controller
{
    ...
    public function update(UpdateNewsRequest $request, $id)
    {
        ...
        if (Auth::user()->role->id == '3' && $request->status == '0') {
            return redirect()->route('admin.newses.index')->with('success', __('message.news-update'));
        } else if (Auth::user()->role->id == '3' && $request->status == '1') {
            return redirect()->route('admin.newses.pending')->with('success', __('message.news-update'));
            ...
            return redirect()->route('superadmin.newses.pending')->with('success', __('message.news-update'));
        } else if (Auth::user()->role->id == '1' && $request->status == '0') {
            return redirect()->route('serveradmin.newses.index')->with('success', __('message.news-update'));
        } else if (Auth::user()->role->id == '1' && $request->status == '1') {
            return redirect()->route('serveradmin.newses.pending')->with('success', __('message.news-update'));
        }
    }
